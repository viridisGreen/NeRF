...
def load_model(net, #* main里make_network，传给train，再传到这里
               optim, #* train里make出来的
               scheduler, #* train里make出来的
               recorder, #* train里make出来的
               model_dir, #* cfg里带的
               resume=True, #* cfg里带的：bool值，代表是否加载继续训练模型
               epoch=-1):
    #* 如果不用继续训练模型，删除整个模型目录，开始新的训练时使用
    if not resume:
        os.system('rm -rf {}'.format(model_dir))

    #* 如果模型目录不存在（肯定用不了继续训练模型了），就直接返回
    if not os.path.exists(model_dir):
        return 0 #* 从第0个epoch开始训练

    #* 使用继续训练模型的时候才回执行以下代码
    pths = [
        int(pth.split('.')[0]) for pth in os.listdir(model_dir)
        if pth != 'latest.pth'
    ] #* 枚举模型目录的所有文件并解析出他们的训练轮次（指[0]）
    if len(pths) == 0 and 'latest.pth' not in os.listdir(model_dir):
        return 0 #* 如果没有训练文件，也没有最新模型，从第0个epoch开始训练
    if epoch == -1:
        if 'latest.pth' in os.listdir(model_dir):
            pth = 'latest' #* 加载最新模型
        else:
            pth = max(pths) #* 加载训练epoch最多的模型
    else:
        pth = epoch
...
def load_pretrain(net, model_dir):
    #? 感觉似乎是实现了load_model里的load net的部分？
